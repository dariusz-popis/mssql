...
select 
       row_number() over (partition by day(DataSprzedazy)%2 order by DataSprzedazy)
          Nr
     , dense_rank() over(order by Ilość desc)
          MiejsceWTabeli
     , sum(Wartość) over(partition by DataSprzedazy order by Ilość desc) 
         SumaWartosci
     , Wartość
     , lag(Wartość,1,0) over(order by DataSprzedazy) 
         WartośćWPoprzednimOkresie
     , lead(Wartość,1,0) over(order by DataSprzedazy) 
         WartośćWNastepnymOkresie
     , ntile(3) over(order by Ilość desc) 
         Strona
     , * 
  from Sprzedaz
